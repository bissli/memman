...
def _vec_512(*prefix: float) -> list[float]:
    """Build a 512-dim vector for cross-backend embedding tests.

    SQLite stores embeddings as BLOB and accepts any dimension; Postgres
    uses `vector(512)` and rejects shorter vectors. Padding with zeros
...
class TestTemporalBackboneChain:
    ...
    def test_temporal_backbone_chain(self, backend):
        ...
        count = create_temporal_edge(backend, ins2)
        assert count >= 2

        edges = backend.edges.by_node_and_type('t-2', 'temporal')
        assert len(edges) >= 2
...
class TestTemporalProximityDecay:
    ...
    def test_temporal_proximity_decay(self, backend, backend_kind):
        ...
        edges = backend.edges.by_node_and_type('tp-3', 'temporal')
        proximity_edges = [
            e for e in edges
...
class TestEntityCoOccurrence:
    ...
    def test_entity_co_occurrence(self, backend):
        ...
        count = create_entity_edges(backend, ins2)
        assert count >= 2

        edges = backend.edges.by_node_and_type('e-2', 'entity')
        assert len(edges) >= 1
...
class TestEntityNoShared:
    ...
    def test_entity_no_shared(self, backend):
...
class TestEntityHyphenatedUserProvided:
    ...
    def test_user_provided_hyphenated_entity_creates_edges(self, backend):
...
class TestEntityIdfWeightedEdges:
    ...
    def test_rare_entity_high_weight(self, backend):
        ...
        backend.nodes.insert(rare)
        target = make_insight(
            id='idf-rare2', content='also rare',
            entities=['UniqueEntity', 'Python'])
        backend.nodes.insert(target)

        create_entity_edges(backend, target)

        edges = backend.edges.by_node_and_type('idf-rare2', 'entity')
        rare_edges = [
            e for e in edges
...
class TestBFSBasic:
    ...
    def test_bfs_basic(self, backend):
        ...
        backend.nodes.insert(make_insight(id='a', content='node A'))
        backend.nodes.insert(make_insight(id='b', content='node B'))
        backend.nodes.insert(make_insight(id='c', content='node C'))
        backend.nodes.insert(make_insight(id='d', content='node D'))
        ...
        result = bfs(backend, 'a', BFSOptions(max_depth=3, max_nodes=0))
...
class TestBFSMaxHops:
    ...
    def test_bfs_max_hops(self, backend):
        """BFS with max_depth=1 finds B but not C (2 hops away)."""
        backend.nodes.insert(make_insight(id='h-a', content='node A'))
        backend.nodes.insert(make_insight(id='h-b', content='node B'))
        backend.nodes.insert(make_insight(id='h-c', content='node C'))
        ...
        result = bfs(backend, 'h-a', BFSOptions(max_depth=1, max_nodes=0))
        ids = {r['insight'].id for r in result}
        assert 'h-b' in ids
...
class TestBFSMaxNodes:
    ...
    def test_bfs_max_nodes(self, backend):
        """BFS with max_nodes=1 returns at most 1 node."""
        backend.nodes.insert(make_insight(id='m-a', content='node A'))
        backend.nodes.insert(make_insight(id='m-b', content='node B'))
        backend.nodes.insert(make_insight(id='m-c', content='node C'))
        ...
        result = bfs(backend, 'm-a', BFSOptions(max_depth=3, max_nodes=1))
        assert len(result) == 1
...
class TestBFSSkipsDeleted:
    ...
    def test_bfs_skips_deleted(self, backend):
        """BFS does not return soft-deleted neighbor."""
        backend.nodes.insert(make_insight(id='sd-a', content='node A'))
        backend.nodes.insert(make_insight(id='sd-b', content='node B'))
        backend.nodes.insert(make_insight(id='sd-c', content='node C'))
        ...
        backend.nodes.soft_delete('sd-b')

        result = bfs(backend, 'sd-a', BFSOptions(max_depth=3, max_nodes=0))
        ids = {r['insight'].id for r in result}
...
class TestSemanticEdgesHighCosine:
    ...
    def test_semantic_edges_high_cosine(self, backend):
        ...
        vec1 = _vec_512(1.0, 0.0, 0.0, 0.0)
        vec2 = _vec_512(0.99, 0.01, 0.0, 0.0)
        backend.nodes.update_embedding('sv-1', vec1, 'voyage-3-lite')
...
class TestSemanticEdgesLowCosine:
    ...
    def test_semantic_edges_low_cosine(self, backend):
        ...
        vec1 = _vec_512(1.0, 0.0, 0.0, 0.0)
        vec2 = _vec_512(0.0, 1.0, 0.0, 0.0)
        backend.nodes.update_embedding('sl-1', vec1, 'voyage-3-lite')
...
class TestSemanticEdgesNoEmbedding:
    ...
    def test_semantic_edges_no_embedding(self, backend):
...
class TestFastEdgesEngine:
    ...
    def test_fast_edges_creates_edges(self, backend):
        ...
        result = fast_edges(backend, ins2)
...
class TestIterEmbeddingsAsVecs:
    ...
    def test_iter_embeddings_as_vecs(self, backend):
        ...
        vec1 = _vec_512(1.0, 2.0, 3.0)
        vec2 = _vec_512(4.0, 5.0, 6.0)
        backend.nodes.update_embedding('bc-1', vec1, 'voyage-3-lite')
        backend.nodes.update_embedding('bc-2', vec2, 'voyage-3-lite')

        cache = dict(backend.nodes.iter_embeddings_as_vecs())
        assert cache
        assert 'bc-1' in cache
        assert 'bc-2' in cache
        assert len(cache['bc-1']) == 512
...
class TestIterEmbeddingsAsVecsEmpty:
    ...
    def test_iter_embeddings_as_vecs_empty(self, backend):
...
class TestEdgeWorthyFiltering:
    ...
    def test_tech_dict_allowed(self, backend):
    ...
    def test_camelcase_allowed(self, backend):
    ...
    def test_url_allowed(self, backend):
