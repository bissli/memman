...
class TestRerank:
    """Voyage rerank() helper."""

    def test_rerank_returns_index_score_pairs(self, monkeypatch):
        """rerank() returns sorted (index, score) tuples from /v1/rerank.
        """
        ...
        out = voyage.rerank('q', ['doc-a', 'doc-b'], top_k=2)
        ...
        assert captured['json']['model']  # populated from default or env

    def test_rerank_empty_documents_returns_empty(self, monkeypatch):
        """rerank() short-circuits on empty document list (no HTTP call).
        """
        ...
        monkeypatch.setitem(
            ...
            type('FakeClient', (),
                 {'post': staticmethod(mock_post)})())
        assert voyage.rerank('q', []) == []

    def test_rerank_raises_on_error_status(self, monkeypatch):
        """rerank() raises RuntimeError on non-200 status."""
        ...
        with pytest.raises(RuntimeError, match='503'):
            voyage.rerank('q', ['d1'], top_k=1)

    def test_rerank_uses_configured_model(self, monkeypatch):
        """MEMMAN_VOYAGE_RERANK_MODEL overrides the default model id.
        """
        monkeypatch.setenv('VOYAGE_API_KEY', 'rk-4')
        monkeypatch.setenv(
            'MEMMAN_VOYAGE_RERANK_MODEL', 'rerank-2.5')
        captured: dict = {}
