...
- **Recall guide**: Default recall on every new user message. Use `memman recall "<query>" --limit 5` (the cross-encoder reranker runs by default on multi-token queries; auto-skipped on 1-2 token queries). Also check the global store: `memman recall "<query>" --store global --readonly --limit 5`. Craft focused keyword-rich queries.
