...
- **The binary is a fast queue-append** (~50 ms). The cost that would justify offloading to a sub-agent (enrichment, embedding) does not run in-band - it runs in the scheduler worker out of band. The host turn pays only the queue-append latency.
