...
`memman remember` appends one row to the queue in ~50 ms. A user-scope scheduler (systemd timer on Linux, launchd agent on macOS) invokes the hidden worker `memman scheduler drain --timeout 60` every 60 s; the drain runs reconciliation and enrichment out of band.
...
4. **Four reconcile stages**: embed via the store's bound provider, then a three-rung shortlist of up to `MAX_SIMILAR_FOR_RECONCILE` (20) rows: up to `KEYWORD_HITS_LIMIT` (5) keyword hits; then up to `RECONCILE_COSINE_SLOTS` (9) rows by cosine, no floor; then the cross-encoder's order over the top `RERANK_POOL` (100) rows by cosine until the cap, the reranker being the read path's (`MEMMAN_RERANK_PROVIDER`, `rerank-3-lite`), governed by the same `MEMMAN_RERANK_ENABLED_<store>` and `MEMMAN_RERANK_ENABLED` toggles. Each rung skips rows already taken; a slot the keyword rung leaves empty goes to the rerank rung; a failed, disabled or exhausted rerank leaves the cosine order to fill the list, and every outcome is traced as `reconcile_rerank` (`status` ok, failed, disabled or skipped; `pool`; `taken`, the rows the rerank rung placed). The screen then sees the reranker's top `RERANK_SCREEN_SLOTS` (10) scored rows plus every row the reranker did not score (a keyword hit outside the pool; every row when the rerank failed or was disabled): a rank quota, measured over 534 case-reps on two model tiers, under which no confirmed target sat below rerank rank 10 and every gate line held its uncut value at 10.7 screen calls per write against 20. A row the cut leaves unscreened carries no relation in the `reconcile-candidates` row; the `reconcile_screen` trace carries `shortlist`, `rows` (screened), `cut` and `kept`. There is no absolute cosine anywhere on the write path; the pool keeps positive cosines only, the sign boundary `vector_anchors` keeps on the read path, because an orthogonal row is orthogonal under every model. The quotas are measured, not tuned: over 64 frozen cases every cosine slot count from 6 to 12 reaches 6 of the 8 targets the cap-10 list missed with all 59 reached kept, and 9 is the middle of that plateau; a swap of the embedding or rerank model re-measures the plateau. An **exact-match dedup rung** runs first: when exactly one shortlist row matches the write byte-for-byte (modulo case and whitespace), the write skips with no LLM call, the stored row's `corroboration_count` is bumped, and a `reconcile-corroborate` oplog row is written; two identical stored rows go on to the stages (the store is already inconsistent, and which row to merge into is exactly the judgment worth a call). The stages run on the `fast_worker` role (the `MEMMAN_LLM_MODEL_FAST` model at the worker's token budget and timeout):
   - **Stage 1, the screen** (`screen_memory`, stage `screen`): every shortlisted row against the write, one pair per call, all rows in parallel on an executor sized to the shortlist. The model names the relation, `CONTRADICTS | REFINES | RESTATES | UNRELATED`, and quotes the clauses of the row the write overturns. An error, an unparsed body or a relation outside the vocabulary reads `UNJUDGED`. The keep rule (`SCREEN_DISPOSITION`) shows CONTRADICTS, RESTATES and UNJUDGED rows to stage 2; when none is kept, the first REFINES row alone; UNRELATED rows are dropped. An empty kept set stores the write as ADD with no further call.
   - **Stage 2, the verdict** (`judge_memory`, stage `reconciliation`): each kept row shown alone under `[0]`, verdict only, no merged text. The first entry with a known token that names a non-null id decides the row (the model numbers sections of the one row and judges a section, so every id it names is that row): SUPERSEDE, UPDATE or NONE, with DELETE read as SUPERSEDE; an ADD there is `keep`, no write against the row. An entry with a null id or a token outside the vocabulary is skipped, so the next known entry decides; an error, an unparsed body or no deciding entry is `keep`. `VERDICT_DISPOSITION` maps every token the text offers, and a test pins its keys to the text's `takes only` line, so a token added to the text without a row fails the suite. `assemble_verdicts` reads the kept rows in shortlist order: every `supersede` row is a target; a `none` row folds to `update` when another row supersedes; the update slot takes the first `update` or folded row alone; with no linking row, the first `none` row is the answer; else ADD. One write's verdicts can name several targets at once - several `supersede` rows plus one folded `update` - and the planner fans that into one plan per target, so one write can leave several near-duplicate active rows; a subsequent write reconciles them as ordinary stored rows.
   - **Stage 3, the merge** (`merge_successor`, stage `merge`): one call per retiring target, the body that target alone with the clauses stage 1 quoted for it (`(none)` for an update target, and for a supersede target the screen did not call CONTRADICTS), then the write. The returned text is that target's successor content; an error, an unparsed body or an empty text falls back to the write's own text and the apply step marks that target's oplog row `(unmerged)`.
   - **Stage 4, apply**: one successor per retiring target. The planner fans a linking result into one plan per target; `_apply_plan` supersedes the target, moves its edges to its own successor, inserts the successor, and writes one `reconcile-update` or `reconcile-supersede` oplog row. UPDATE and SUPERSEDE retire the target the same way; SUPERSEDE alone withholds the `corroboration_count` carry. A target that is not current by apply time is dropped and the write degrades to a plain add; that drop writes a `target-gone` oplog row against the successor, naming the requested target, because the row IS stored and so is never filed in the skipped-write ledger, leaving `memman log list` the only place a caller can learn a correction did not attach. The shortlist is written once per write as a `reconcile-candidates` oplog row (`fact_id`, the write's text, and each candidate with its rung, score, stage-1 relation, whether stage 2 saw it, and its verdict) so the decision can be replayed against exactly what each stage saw.

   A `NONE` verdict is the second corroboration route: the model names the memory that already captures the write, the plan carries that id, and the write is skipped with the named row bumped exactly as the rung does it. The verdict text's `NONE <id>` line keeps its id demand: without it the model answers NONE with a null target, the reader drops an id-less verdict, and the restatement is stored as a new row instead of bumping the named one (see 02-concepts.md, Corroboration semantics). The rung and the stages do not run under `--no-reconcile` (verbatim-store contract; `replace` routes through that flag). Three refinements bound the bump: the restating row's `queue_uuid` is adopted only when the target carries none (the creating row's replay guard outranks the restating row's); a target no longer current between planning and apply (forgotten, or superseded by an earlier write) degrades the skip to a plain add carrying the already-computed vector (the write is never dropped, the dead row is never bumped); and one queue row bumps a given target at most once. The skip result names the corroborated row via `target_id`.

   The screen and merge texts ship byte for byte as the probe measured them; the verdict text is the measured text without its several-memories tie-break sentence, which has no referent when one row is shown. `tests/test_reconcile_stages.py` pins each text by hash. The recall page for one write can now hold N successors, one per retired row; the F2 round counts retiring targets per write from the `reconcile-candidates` rows.
...
6. **Re-embed** with enriched keywords; rebuild auto edges.
7. `mark_done(queue_id)` on success, or `mark_failed` (retry up to 5 times across stale-claim windows before status='failed').
8. **Skipped-write ledger**: a row that stored nothing (reconciliation matched the write to an existing insight - the exact-match rung, or a `NONE` verdict - and folded it in as a corroboration) is filed in the `skipped_writes` sidecar table with its full content, the reason, the store, and the session id, then marked `done` like any other row. The sidecar exists because `skipped` is not a queue status and cannot become one: `_migrate` carries no `alter` migrations, so a widened CHECK constraint is a no-op on every existing database and writing the new status would raise inside the drain's try block, routing the row to `mark_failed`. A row that exhausted its retries and parked at `status='failed'` usually stored nothing either, so `queue purge --failed` files it here on the way out, carrying its own `processed_at` so it keeps its place in the newest-first order; its reason names the failure rather than a pipeline skip, and carries the row's `queue_uuid`, because the drain's error handling extends past the store commit and a row can therefore reach `failed` with its insight already written. The uuid is what tells the two cases apart once the queue row is gone. `purge_done` deletes the queue row after 60 s and never reaches the ledger; `memman scheduler queue skipped` reads it back, `queue purge --skipped` empties it, `purge_store` drops one store's entries, and a `backup restore` replaces it wholesale. Nothing prunes it on a timer: a retention window would be a tunable, and memman does not ship estimated tunables.
...
An UPDATE, SUPERSEDE or REPLACE is not an in-place edit. `_apply_plan` supersedes the plan's target and inserts one **successor** row per plan, so a write that retires several rows leaves one successor per row: each target keeps its content behind `superseded_by` and leaves every active read, so every field of the current view is decided by one of two rules, and a field governed by neither stays only on the predecessor. Every predecessor's pointer is written before the successor is inserted, and that order is load-bearing: the temporal builder reads the session's latest row, so the predecessors must already be out of the active set or the successor chains its backbone to a row it replaced. The split:

| Field                                          | Winner                                               | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content`                                      | incoming (the stage-3 merge text for this target)    | the merge is what the caller asked for; the write's own text when no merge came back; the caller's text on a replace, which runs no stage                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `entities`                                     | **union** of both, capped at `MAX_ROW_ENTITIES` (50) | enrichment sees only the incoming text, so overwriting would narrow the set on every merge; the union is monotonic, so the cap bounds a repeatedly superseded row. The row's own list leads, so the cut normally takes the oldest inherited names -- the same order the edge budget is spent in, and the ones whose carriers are superseded and so match no active row. Where that own list already fills the cap the cut lands inside it, on the enrichment's names, which sit at its tail. The cap does not bound what a row holds: the enrichment adds up to `MAX_ENRICH_ENTITIES` on top of whatever seeds it, and a `replace` carries the caller's list as given, including a list inherited over the cap, which the next reconciliation trims |
| `access_count`                                 | **max** of both                                      | earned recall history must survive a rewording                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `corroboration_count`                          | **max** on UPDATE and REPLACE; incoming on SUPERSEDE | restatements of a claim do not corroborate the write that falsified it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
Every `MemmanLLMClient.complete` call names its originating stage from a closed set (`screen`, `reconciliation`, `merge`, `query_expansion`, `enrichment`, `probe`, plus `harness` for off-pipeline measurement tooling - an unknown stage raises). The client reads the provider's `usage` block **per attempt inside the retry loop** - an empty HTTP-200 body retried twice is three billed completions, so success-only accounting undercounts - and accumulates into a process-wide ledger behind a `threading.Lock` (the drain screens rows concurrently, so event-order attribution is unrecoverable). The drain worker snapshots the ledger per row (each `queue_done`/`queue_failed` trace event carries the row's per-stage delta) and emits an `llm_usage_summary` trace event plus an `llm_usage` key in the drain's JSON output with the drain-level totals. An HTTP-200 response with no `usage` block counts the call under `missing_usage` without inventing zero tokens; non-2xx attempts land in `http_errors` rather than `calls`, so a retried rate-limit storm cannot inflate the billed-call signal, and an HTTP-200 whose body is not JSON is booked like an empty body and retried.
...
- `MEMMAN_LLM_MODEL_FAST` - recall hot path, `doctor`'s connectivity probe, and, as the `fast_worker` role at the worker's token budget and timeout, the three reconcile stages (screen, verdict, merge).
- `MEMMAN_LLM_MODEL_SLOW_METADATA` - worker's derived-metadata path (enrichment summaries/keywords).
...
| `memman scheduler queue purge --failed`   | file failed rows into the ledger, then delete them                                         |
...
memman calls LLMs at write time (reconciliation, enrichment) and embedding models on every vector. Prompts get edited, models get upgraded, providers get swapped. The design goal is detection and re-run, not bit-identical output across versions.
...
| `insights.prompt_version`                                        | per row   | enrichment prompt or `slow_metadata` model change                                                                              | `memman doctor` warns; remediate via `memman graph rebuild --stale-only` or `UPDATE insights SET linked_at=NULL, enriched_at=NULL WHERE prompt_version='<old>';` then drain. `insights.model_id` is write provenance only and is NOT a drift signal: it names the model behind the row's content, which no rebuild rewrites. |
