...
memman never switches the model on its own. On an OpenRouter endpoint the install wizard and `memman config models` list up to three candidates and write the one the operator picks. A candidate is a general-purpose model (no `-vl-`, `coder`, `:free` or `:batch` variant) in the current model's family, served under zero data retention by a vendor in `MEMMAN_LLM_PROVIDER_ONLY`, inside both `MEMMAN_LLM_MAX_INPUT_PRICE` and `MEMMAN_LLM_MAX_OUTPUT_PRICE` (dollars per million tokens). The list runs dearest first, newest first on a tie, and each row shows the price, the vendor, the release date and whether the model thinks by default. The two catalogs it joins, `/endpoints/zdr` and `/models`, are public and need no key. For any non-OpenRouter endpoint the install wizard prompts for the slug (vendor-native model ids like `gpt-4o-mini` or `qwen2.5:7b` don't share OpenRouter's `provider/model` slug shape), and a headless install with no model refuses. Runtime never queries the model inventory; it reads the persisted id and sends it through unchanged.
