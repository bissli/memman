...
- `MEMMAN_LLM_MODEL_SLOW_CANONICAL` - worker's canonical-content path (fact extraction).
...
memman calls LLMs at write time (extraction, reconciliation, enrichment) and embedding models on every vector. Prompts get edited, models get upgraded, providers get swapped. The design goal is detection and re-run, not bit-identical output across versions.
