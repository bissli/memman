...
It is a fast queue-append (~50 ms). A scheduler trigger drains the queue and runs the full pipeline — fact extraction, reconciliation, enrichment, causal inference, embedding — out of band. The trigger varies by environment: a `systemctl --user` timer on Linux hosts, a launchd agent on macOS hosts, or an in-process inline drain inside nanoclaw containers (no systemd/launchd available). On hosts the host agent calls `memman remember` directly via Bash; newly stored memories are NOT recallable in the current session — they become available after the worker fires. Inside containers (inline trigger), the drain runs in-process before `remember` returns, so the row is visible immediately.

**How do I stop the scheduler?**
`memman scheduler stop` deactivates the trigger without removing unit files. `memman scheduler start` re-arms it. When stopped, memman is **recall-only**: `remember`, `replace`, `forget`, `graph link`, `graph rebuild`, and `insights protect` reject with `Scheduler is stopped; cannot <verb>. Run 'memman scheduler start' to enable.` `memman scheduler interval --seconds N` changes the cadence (min 60 s). `memman scheduler trigger` runs the drain immediately under the scheduler's environment (rejects when stopped). `memman log worker` prints the tail of the enrichment worker's output; add `--errors` to see stderr and tracebacks. Logs live at `~/.memman/logs/enrich.{log,err}`.
