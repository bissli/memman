...
`memman scheduler pause` stops the timer/agent without removing unit files. `memman scheduler enable` resumes. `memman scheduler disable` removes the units entirely (and `remember` defaults to `--sync`). `memman scheduler interval --seconds N` changes the cadence (min 60 s). `memman scheduler trigger` runs the drain immediately under the scheduler's environment, outside the normal interval. `memman log worker` prints the tail of the enrichment worker's output; add `--errors` to see stderr and tracebacks. Logs live at `~/.memman/logs/enrich.{log,err}`.
