...
| `MEMMAN_LLM_MODEL`              | installed | Worker model for derived metadata (enrichment summaries/keywords). The install seeds `qwen/qwen3-235b-a22b-2507` on OpenRouter and prompts for a slug on other endpoints; the operator changes it with `memman config set`.         |
...
Model selection comes from `MEMMAN_LLM_MODEL`, and only the operator changes it. On OpenRouter endpoints `src/memman/llm/openrouter_models.py` checks the model against the public `/endpoints/zdr` and `/models` catalogs: a zero-data-retention endpoint on a vendor in `MEMMAN_LLM_PROVIDER_ONLY` must serve it, and OpenRouter must carry no retirement date for it. The install runs the check at once, and the scheduler drain runs it once a day and records the verdict in `<data_dir>/model.state`, which `memman prime` reads. For non-OpenRouter endpoints the install wizard prompts for the slug.
