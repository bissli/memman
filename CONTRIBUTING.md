...
- Process-control variables (`MEMMAN_DATA_DIR`, `MEMMAN_STORE`, `MEMMAN_WORKER`, `MEMMAN_DEBUG`, `MEMMAN_SCHEDULER_KIND`, `MEMMAN_AUTHOR`) are read directly from `os.environ` and excluded from the env-file model.
...
| `MEMMAN_LLM_MODEL`              | installed | Worker model for derived metadata (enrichment summaries/keywords). The operator picks it from `memman config models` or the install wizard on OpenRouter; other endpoints are prompted interactively.                               |
| `MEMMAN_LLM_MAX_INPUT_PRICE`    | installed | Input price ceiling, dollars per million tokens, for the candidates `memman config models` lists (default `0.25`).                                                                                                                  |
| `MEMMAN_LLM_MAX_OUTPUT_PRICE`   | installed | Output price ceiling, dollars per million tokens, for the candidates `memman config models` lists (default `1.00`).                                                                                                                 |
...
Model selection comes from `MEMMAN_LLM_MODEL`, and only the operator changes it. On OpenRouter endpoints `src/memman/llm/openrouter_models.py` lists up to three candidates from the public `/endpoints/zdr` and `/models` catalogs for the install wizard and `memman config models`; for non-OpenRouter endpoints the install wizard prompts for the slug.
