...
Per-role model selection comes from `MEMMAN_LLM_MODEL_FAST` / `_SLOW_METADATA`. For OpenRouter endpoints these are resolved against `/v1/models` at install time; for non-OpenRouter endpoints the install wizard prompts interactively for each slug.
